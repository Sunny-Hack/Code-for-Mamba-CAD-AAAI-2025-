...
class TrainerLatentWGAN(BaseTrainer):
    ...
    def set_optimizer(self, cfg):
        ...
        self.optimizerD = optim.Adam(self.netD.parameters(), lr=2e-4, betas=(cfg.beta1, 0.9))
        self.optimizerG = optim.Adam(self.netG.parameters(), lr=2e-4, betas=(cfg.beta1, 0.9))
    ...
    def calc_gradient_penalty(self, netD, real_data, fake_data):
        #alpha = torch.rand(self.batch_size, 1)original
        alpha = torch.rand(self.batch_size, 64, 1)
    ...
    def train(self, dataloader):
        ...
        for iteration in pbar:
            ...
            for iter_d in range(5):
                real_data = next(data)
                '''
                data_mean = real_data.mean()
                data_std = real_data.std()
                ...
                gradient_penalty = self.compute_gradient_penalty(self.netD, real_data.data, fake.data)
                #D_cost = -torch.mean(torch.log(D_real) + torch.log(1 - D_fake)) + 10 * gradient_penalty
                D_cost = -torch.mean(D_real) + torch.mean(D_fake) + 20 * gradient_penalty
                D_cost.backward()
                Wasserstein_D = D_real.mean(dim=0, keepdim=True) - D_fake.mean(dim=0, keepdim=True)
                self.optimizerD.step()
                ...
                for _ in range(2):
                    self.netG.zero_grad()
                    fake_preds = self.netD(fake)
                    G_cost = -torch.mean(fake_preds)
                    #G_cost.backward()
            ...
            noise = torch.randn(self.batch_size, self.n_dim, 128)
            noise = noise.cuda()
