...
def reconstruct(cfg):
    ...
    for i, data in enumerate(pbar):
        ...
        commands_ = gt_vec[:, :, 0]
        # #random mamba test
        # arg_mask = data['args'] == -1
        # random_args = torch.randint(0, 256, data['args'].size(), dtype=torch.long)
        # data['args'] = torch.where(arg_mask, data['args'], random_args)
        # data['command'] = torch.randint(0, 6, data['command'].size())
        # #random mamba test
        with torch.no_grad():
